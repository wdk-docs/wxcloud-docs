...
... 数据库容量达到上限之后怎么办？
  ...
  如果数据库的剩余存储容量低于 10%，则会通过短信、站内信、邮件等方式进行告知，
  您可以升级您的环境版本，以享有更多的数据库存储容量，避免影响业务的正常运行。
...
... 为什么数据库中的部分数据在控制台可以看到，但是在客户端却获取不到？
  ...
  如果是在控制台添加的数据，属于管理员添加的数据，没有 _openid 信息，因此不属于任何一个用户。
  这部分数据只能由管理员通过控制台进行编辑，而不支持通过客户端进入写入操作（即使管理员，也不能通过客户端进行写入操作）。
